...
class Subjects:
    ...
    def get_subject(cls):
        ...
        results = connectToMySQL('enrollment').query_db(query)
        subjects = []
    ...
    @classmethod
    def update_subject(cls, data):
        query = "UPDATE subjects SET course_code=%(course_code)s, description=%(description)s, semester_offered=%(semester_offered)s, updated_at=NOW() WHERE id = %(id)s"
        return connectToMySQL('enrollment').query_db(query, data)
